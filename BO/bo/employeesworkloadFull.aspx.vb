...
Partial Class BO_employeeworkloadFull
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Not Me.IsPostBack Then
            'loadRole(eempID)
            Me.CPDate.SelectedDate = Now.AddDays(-1)
            Me.CPDate.VisibleDate = Now.AddDays(-1)
    ...
    Protected Sub btnSearch_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnSearch.Click
        ...
        Query = " SELECT case(dicL.empID) when '0' then '-' else dicl.empID end AS empID," _
              ...
              & " case when empDesignation IS NULL or empDesignation = '' then '999' else substring(empDesignation,1,2) end as EmpDesignation " _
              ...
              & " WHERE (" & QueryClause2 & " BETWEEN '" & Format(Me.CPDate.SelectedDate, "MM/dd/yyyy") & "' AND '" & Format(Me.CPTo.SelectedDate, "MM/dd/yyyy") & "') GROUP BY dicL.empID, Emp.empFirstName, Emp.empLastName,EmpDesignation " _
              & " ORDER BY EmpDesignation, empFirstName, empLastName;"
    ...
    Protected Function ifPerDone(ByVal mins As Double, ByVal done As Double, ByVal curr_mins As String, curr_dics As String) As String
        Try
            Dim donePer As String
            donePer = Math.Round(((done / mins) * 100), 0, MidpointRounding.AwayFromZero)
            If donePer = "100" Then
                Return "<span style='color:green !important;'>" & curr_mins & "&nbsp;&nbsp;&nbsp;&nbsp;[<Lable style='color:grey;'>" & curr_dics & "</Lable>]</span>"
            Else
                Return "<span>" & curr_mins & "&nbsp;&nbsp;&nbsp;&nbsp;[<Lable style='color:grey;'>" & curr_dics & "</Lable>]</span>"
            End If
        Catch ex As Exception
            Return "<span>" & curr_mins & "&nbsp;&nbsp;&nbsp;&nbsp;[" & curr_dics & "]</span>"
        End Try
    End Function

    Protected Function CheckCapcity(ByVal totalminutes As String, ByVal minutes As String, ByVal status As String, ByVal firstname As String, ByVal lastname As String) As String
        Try
            Dim capcityhtml As String = ""
            Dim total_minutes As String = "" 
            If status = "0" Then
                total_minutes = firstname + " " + lastname
            Else
                total_minutes = minutes
            End If

            Dim arr() As String = Split(minutes, ":")
            If CType(totalminutes, Decimal) = CType(arr(0), Decimal) Then
                capcityhtml = "<span style='color:green'> " & total_minutes & "</span>"
            ElseIf CType(totalminutes, Decimal) < CType(arr(0), Decimal) Then
                capcityhtml = "<span style='color:Red'> " & total_minutes & "</span>"
            Else
                capcityhtml = "<span> " & total_minutes & "</span>"
            End If
            Return capcityhtml
