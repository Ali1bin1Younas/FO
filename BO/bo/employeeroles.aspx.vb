...
Partial Class roles
    ...
    Private Sub LoadGrid()
        ...
        Dim ds As DataSet
        ds = Con.getDataSet("SELECT RolId,rolDescription FROM boRoles WHERE rolEnable=1 Order By rolOrder; Select empID, rolID, IsNull(empRolCapacity, '') as empRolCapacity,empEnable  from boEmployeeRoles Where empId='" + Request.QueryString("empID") + "'")
        Dim dGrid As New DataTable
        dGrid.Columns.Add("RolID")
        dGrid.Columns.Add("rolDescription")
        dGrid.Columns.Add("empID")
        dGrid.Columns.Add("empRolCapacity")
        ...
        For Each dsRow As DataRow In ds.Tables(0).Rows
            ...
            dGridRow.Item("empRolCapacity") = Me.EmpRoleCapacity(ds.Tables(1), dsRow.Item("rolID"), Request.QueryString("empID"))
    ...
    Protected Sub btnSave_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnSave.Click
        ...
        Finally
            ...
            conn.Dispose()
        End Try
        Response.Redirect("employeeroles.aspx?empID=" & eempID & "")
    End Sub
    ...
    Protected Function EmpRoleCapacity(ByVal dt As DataTable, ByVal rol As String, ByVal empID As String) As String
        Dim dRow() As DataRow
        Dim rolCap As String = ""
        Try
            dRow = dt.Select("RolID='" & rol & "' and empId='" & empID & "'")
        Catch ex As Exception
        ...
        If dRow.Length > 0 Then
            ...
            For Each row As DataRow In dRow
                rolCap = CType(row.Item("empRolCapacity"), String)
            Next
        End If
    ...
    <WebMethod> _
    Public Shared Function updateEmpRolCapacity(ByVal cap As String, ByVal empID As String, ByVal rolID As String) As String
        Dim objDB As New DBTaskBO
        Dim qry As String = ""
        Try
            qry = "update boEmployeeRoles set empRolCapacity='" & cap.Trim() & "' where empID='" & empID.Trim() & "' and rolID='" & rolID.Trim() & "'"
            objDB.update(qry)
            Return "1"
        Catch ex As Exception
            Return "-1"
