...
Partial Class BO_singleassign3
    ...
    Protected Sub GridView1_RowCreated(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewRowEventArgs)
        ...
        If e.Row.RowType = DataControlRowType.DataRow And Me.BtnClick Then
            ...
            Dim dicTempName As String = Me.GridDetial.DataKeys(e.Row.RowIndex).Values.Item(5)
            ...
            End If
            ''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
            Dim hVoice As HyperLink = CType(e.Row.FindControl("btnPlayDic"), HyperLink)
            hVoice.NavigateUrl = "../dictationpath.aspx?tempName=" & dicTempName
            hVoice.Target = "_blank"
            End If
    ...
    Public Shared Function get_dic_trans(ByVal dicID As String) As String
        Try
            ...
            Qry = "Select tD.dicID, tD.drID, tD.foID, tT.traID,tT.traSubject,tT.traClinicDate,tT.traNTS,tT.traRecipientAddress,tT.traIncomplete,tT.traDear,tT.traNHSNo,tT.traDOB,tT.traNTD,tT.traFooterBlock, tT.traName, isNull(traPatFirstName,'') as traPatFirstName , isNull(traPatLastName,'') as traPatLastName, isNull(traPatientNo,'') as traPatientNo from boDictation tD inner join boTranscription tT on tT.dicID = tD.dicID where tD.dicId=" & dicID
            Dim ds As DataSet = Con.getDataSet(Qry)
    ...
    <WebMethod> _
    Public Shared Function get_dr_rols(ByVal drID As String) As String
        Dim claus As String = "<div style='width:155px;float:left;'>"
        Dim clauseMT As String = "<div style='width:150px;float:left;'>"
        Dim clauseQC As String = "<div style='width:150px;float:left;'>"
        Dim clausePR As String = "<div style='width:150px;float:left;'>"
        Dim clauseFR As String = "<div style='width:150px;float:left;'>"
        Dim mt As String = ""
        Dim qc As String = ""
        Dim pr As String = ""
        Dim fr As String = ""
        ...
        Dim ds As DataSet
        Try
            qry = "select distinct(bE.empID) as empID,(empfirstName +' '+ empLastName) as empName,bDL.rolID, IsNull(COUNT(case(bDL.diclStatus) when 3 then 1 else null end), 0) as doneDictations " _
                ...
                & " inner join boDictationLayers bDL on bDL.dicID = bD.dicID " _
                & " where bDL.diclWorkloadDate between '" & Format(Today.Date.AddMonths(-7), "MM/dd/yyyy") & "' and '" & Format(Now, "MM/dd/yyyy") & "' and drID = '" & drID & "') and bE.empID != '0' " _
                & " group by bE.empID, bE.empFirstName, bE.empLastName, bDL.rolID " _
                & " order by doneDictations desc"
            ds = con.getDataSet(qry)
            If ds.Tables(0).Rows.Count > 0 Then
                For Each dr As DataRow In ds.Tables(0).Rows

                    If dr("empID").ToString().ToLower().Trim() <> "dp0020" AndAlso dr("empID").ToString().ToLower().Trim() <> "000000" AndAlso Not dr("empName").ToString().ToLower().Contains("tag") Then

                        If dr("rolID").ToString().Trim() = "MT" Then
                            If mt <> "" Then
                                clauseMT += "<br/><span>" & dr("empName") & "</span>"
                            Else
                                clauseMT += "<span style='font-weight:bold;color:grey;text-align:center;width:70px;'>MT</span><hr><span>" & dr("empName") & "</span>"
                                mt = "1"
                            End If
                        End If

                        If dr("rolID").ToString().Trim() = "QC" Then
                            If qc <> "" Then
                                clauseQC += "<br/><span>" & dr("empName") & "</span>"
                            Else
                                clauseQC += "<span style='font-weight:bold;color:grey;text-align:center;width:70px;'>QC</span><hr><span>" & dr("empName") & "</span>"
                                qc = "1"
                            End If
                        End If

                        If dr("rolID").ToString().Trim() = "PR" Then
                            If pr <> "" Then
                                clausePR += "<br/><span>" & dr("empName") & "</span>"
                            Else
                                clausePR += "<span style='font-weight:bold;color:grey;text-align:center;width:70px;'>PR</span><hr><span>" & dr("empName") & "</span>"
                                pr = "1"
                            End If
                        End If

                        If dr("rolID").ToString().Trim() = "FR" Then
                            If fr <> "" Then
                                clauseFR += "<br/><span>" & dr("empName") & "</span>"
                            Else
                                clauseFR += "<span style='font-weight:bold;color:grey;text-align:center;width:70px;'>FR</span><hr><span>" & dr("empName") & "</span>"
                                fr = "1"
                            End If
                        End If
