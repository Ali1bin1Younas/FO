...
Partial Class bo_boMacros
    ...
    Private Sub gv_macros_DataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewRowEventArgs) Handles gv_macros.RowDataBound
        If e.Row.RowType = DataControlRowType.DataRow Then
            ...
            Dim img_mcDefault As ImageButton = CType(e.Row.FindControl("img_mcDefault"), ImageButton)
            If CType(dvr("mcDefault").ToString(), Boolean) Then
                img_mcDefault.ImageUrl = "~/Icon/Ngood.gif"
            Else
                img_mcDefault.ImageUrl = "~/Icon/cancel.gif"
            End If
        End If
    End Sub
    ...
    Private Sub btn_upload_macro_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btn_upload_macro.Click
        ...
        Try
            If btn_select_macro.HasFile Then
                Dim str As String = btn_select_macro.FileName.Split(".")((btn_select_macro.FileName.ToString.Split(".").Length - 1))
                If str = "dot" Then
                ...
                Else
                    lbl_error.Visible = True
                    lbl_error.Text = "Please select a valid to upload."
                End If
            End If
        Catch ex As Exception
            lbl_error.Visible = True
    ...
    Protected Sub btnDelTran_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs)
        Dim btn As ImageButton = CType(sender, ImageButton)
        If btn.CommandName = "Changestatus" Then

            Dim Qry As String
            Dim Con As New DBTaskBO

            Qry = "update boMacros set mcDefault='0'; update boMacros set mcDefault='1' where mcID='" + btn.CommandArgument.Trim() + "';"
            Con.saveData(Qry)
            load_macros_grid()
        End If
