...
function make_folder_structure_tree_loop(treeXml, fid, chk, root_path) {
    ...
    treeXml.each(function () {
        ...
        if (fid == temXml.find("parentid").text()) {
            ...
            var folder_html = '<img id="img_del_' + fID + '" class="folder_img_del" src="../icon/cancel.gif" onclick="remove_folder(this,' + fID + ')"/>';

            folder_html += '<input class="folder_chkbox" ' + dis + ' ' + checked + 'type="checkbox" id=chk_"' + fID + '" onclick="enable_folder(this,' + fID + ')"/> ' +
                '<img id="img_' + fID + '" class="folder_img" src="../icon/window.png"/>' +
                '<Label class="folder_lbl">' + temXml.find("foldername").text() + '</Label>' +
            ...
            //condition to show schedule dropdown only for leaf nodes
...
function set_folder_schedule(obj, fID) {
    try {
        if ($(obj).val() != 0) {
            ...
            PageMethods.get_schedule(fID, $(obj).val(), onSuccess_set_folder_schedule);
        }
    } catch (e) {
        alert(e.message.toString());
    }
...
function set_schedule_default(obj) {
    try {
        ...
        var row = tbl_sch_main.getElementsByTagName("tr")[tbl_sch_main.getElementsByTagName("tr").length - 2];
