...
Public Class BL
    ...
    Public Function Load_Employee_Work_Area_Grid(ByVal rol_id As String, ByVal empId As String) As String
        Dim SQry As String
        If rol_id = "MT" Then
            SQry = "SELECT boDictation.drID, isNull(boDictationLayers.diclWorkloadDate, '01-01-1999') as diclWorkloadDate, boDictation.foID,boDictation.dicAccountName, boDictation.dicActualName,  boDictation.dicTempName, boDictation.dicLength, boDictation.dicStatus, boDictationLayers.diclStatus, " _
                    & " boDictation.dicDate,boDictation.dicID,boDictation.dicTime, " _
                    & "boDictation.dicStatus,boDictation.dicID, boDictation.dicUrgent, boDictationLayers.diclStatus FROM boDictation " _
                    ...
                    & "GROUP BY boDictation.dicActualName,boDictationLayers.diclWorkloadDate,boDictation.dictempName,boDictation.dicAccountName ,boDictation.dicLength, boDictation.foID, " _
                    & "boDictation.drID, boDictation.dicDate,boDictation.dicID, boDictation.dicUrgent, " _
        ...
        Else
            SQry = "SELECT boDictation.dicID, isNull(boDictationLayers.diclWorkloadDate,'01-01-1999') as diclWorkloadDate, boDictation.drID, boDictation.dicAccountName, boDictation.foID, boDictation.dicActualName,  boDictation.dicTempName, boDictation.dicDate, boDictation.dicLength, boDictation.dicStatus, boDictationLayers.diclStatus, " _
                        ...
                        & "inner join boTranscription on boTranscription.traID = boTranscriptionLayers.traID " _
                        & "inner join boDictation on boDictation.dicID = boTranscription.dicID " _
                        & "inner join boDictationLayers on boDictationLayers.dicID = boDictation.dicID and boDictationLayers.rolID = boTranscriptionLayers.rolID " _
                        & "and boDictationLayers.empID = boDictationLayers.empID " _
        ...
        Qry = "SELECT boDictation.drID, boDictation.foID, isnull ((select sum(c.dicLength) FROM  boDictation c " _
                ...
                & " isnull ((SELECT Count(c.dicID) FROM  boDictation c INNER JOIN boDictationLayers ON c.dicID = boDictationLayers.dicID WHERE boDictationLayers.empID = '" & empId & "' " _
                & " AND boDictationLayers.rolID = 'MT' AND boDictationLayers.diclStatus IN(1,2) AND c.drID = boDictation.drID and c.foID = boDictation.foID and c.dicUrgent = '1'),0)  AS hasUrgent, " _
    ...
    Public Function Load_Dictators(ByVal empID As String) As String
        Qry = "SELECT DISTINCT(RTRIM(dic.foID)+dic.drID) AS Uid," _
            & " RTRIM(dic.foID) +'-'+dic.drID +' '+ dr.drLastName +', '+ ISNULL(dr.drMiddleName, '') +' '+ dr.drFirstName as DictatorName" _
